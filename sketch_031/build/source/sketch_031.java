...
public class sketch_031 extends PApplet {
...
public void draw() {
  ...
  float fov = map(n,-1,1,-1,1.0f);  //\u8996\u91ce\u89d2
  ...
  for (float v = minV; v <= maxV; v += stepV) {
    for (float u = minU; u <= maxU; u += stepU) {
      float x = ((1-cos(u))*(cos(u+TWO_PI/3))*cos(v+TWO_PI/3)/2) * scale;
      float y = ((1-cos(u))*(cos(u+TWO_PI/3))*cos(v-TWO_PI/3)/2) * scale;
      float z = (cos(u-TWO_PI/3)) * scale;
      pushMatrix();
      translate(x, y, z);
      // stroke(baseColor);
      // point(0,0,0);
...
public void gifDraw(){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_031" };
    if (passedArgs != null) {
