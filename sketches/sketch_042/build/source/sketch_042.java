...
public class sketch_042 extends PApplet {
...
public void draw() {
  ...
  float fov = map(n,-1,1,0.15f,1.0f);  //\u8996\u91ce\u89d2
  ...
  for (float v = minV; v <= maxV; v += stepV) {
    for (float u = minU; u <= maxU; u += stepU) {
      float m3 = map(sin(frameCount/(float)gifCount * TWO_PI),-1,1,1.0f,6.0f);
      float m4 = map(sin(frameCount/(float)gifCount * TWO_PI),-1,1,1.0f,6.0f);
...
public void gifDraw(){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_042" };
    if (passedArgs != null) {
