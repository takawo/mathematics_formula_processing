...
public class sketch_018 extends PApplet {
...
public void draw() {
  ...
  for (float v = minV; v <= maxV; v += stepV) {
    for (float u = minU; u <= maxU; u += stepU) {
      float x = (sqrt(10.0f * v) * cos(u)) * scale;
      float y = (sqrt(10.0f * v) * sin(u)) * scale;
      float z = cos(5.0f * u) * scale;
      pushMatrix();
      translate(x, y, z);
      sphere(5);
      popMatrix();
...
public void gifDraw(){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_018" };
    if (passedArgs != null) {
